...
import { saveBrandCostConfig, type CostConfigResult } from '@/app/actions/cost-config'

const TIERS = ['starter', 'standard', 'professional', 'enterprise'] as const
...
export function CeilingEditor({ brandId, current, labels }: Props) {
  const [open, setOpen] = useState(false)
  const [state, formAction, isPending] = useActionState(saveBrandCostConfig, initial)
      ...
      {open && (
        <div className="mt-3 rounded-(--r-md) border border-(--border-subtle) bg-(--surface-2) p-4">
          <form action={formAction} className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div className="space-y-1">
              <label className="block text-xs uppercase tracking-wide text-(--fg-muted)">{labels.tierLabel}</label>
              <select name="tier" defaultValue={current.tier}
                className="w-full rounded-(--r-md) border border-(--border-default) bg-(--surface-4) px-3 h-9 text-sm text-(--fg) focus:border-(--accent) focus:outline-none focus:ring-2 focus:ring-(--accent-soft)">
